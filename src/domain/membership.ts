...
export function getAvailableModesForMember(
  ...
  if (member.roles.includes(MemberRole.ACCOMMODATION_FELLOWS_COORDINATOR)) {
    return ['FELLOW', 'COORDINATOR', 'CAPTAIN_COVERAGE', 'FINANCIAL_COVERAGE'];
  }
  ...
  if (isCaptain) {
    return ['FELLOW', 'ROOM_CAPTAIN'];
  }

  // Base Fellow: no switcher
  return ['FELLOW'];
}
...
export function formatActionAttribution(
  ...
  if (activeMode === 'FINANCIAL_COVERAGE') {
    return {
      ...
      actingCapacity: 'L2E Accommodation Fellows Coordinator (Financial Administration Coverage)',
      isCoverage: true,
    };
  }

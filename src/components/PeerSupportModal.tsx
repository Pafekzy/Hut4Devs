...
export const PeerSupportModal: React.FC<PeerSupportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-stone-200 animate-in fade-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-stone-100 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-serif font-bold text-stone-900 text-base">Initiate Peer Support</h3>
        ...
        <div className="mt-3.5 p-3 bg-[#FFF9EE] border border-[#E7D6C1] rounded-xl text-[11px] text-[#5A2D0C] space-y-1">
          <div className="font-semibold flex items-center gap-1.5 text-[#5A2D0C]">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-stone-700 mb-1.5">Choose Support Path</label>
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className={`p-2.5 rounded-xl border text-left transition-all text-xs flex flex-col gap-1 cursor-pointer ${
                  supportType === 'gift'
                    ? 'border-emerald-600 bg-emerald-50 text-emerald-950 font-semibold shadow-xs ring-1 ring-emerald-600/30'
              ...
              <button
                ...
                className={`p-2.5 rounded-xl border text-left transition-all text-xs flex flex-col gap-1 cursor-pointer ${
                  supportType === 'loan'
                    ? 'border-blue-600 bg-blue-50 text-blue-950 font-semibold shadow-xs ring-1 ring-blue-600/30'
                    : 'border-stone-200 bg-white text-stone-600 hover:bg-stone-50'
                }`}
              >
              ...
              <button
                ...
                className={`p-2.5 rounded-xl border text-left transition-all text-xs flex flex-col gap-1 cursor-pointer ${
                  supportType === 'contribution'
                    ? 'border-purple-600 bg-purple-50 text-purple-950 font-semibold shadow-xs ring-1 ring-purple-600/30'
                    : 'border-stone-200 bg-white text-stone-600 hover:bg-stone-50'
                }`}
              >
          ...
          {supportType === 'loan' && showDebtHistory && borrowerHistory && (
            <div className="p-3 bg-stone-50 border border-stone-200 rounded-xl text-xs space-y-2 animate-in fade-in duration-150">
              <div className="flex items-center justify-between font-semibold text-stone-800">
                ...
                <span className="text-[10px] text-stone-500 font-normal">Confidential to Prospective Lender</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-[11px]">
                <div className="p-2 bg-white rounded-lg border border-stone-200/80">
                  <span className="text-stone-500 block">Fully Repaid Loans:</span>
                  <strong className="text-emerald-700 text-xs">{borrowerHistory.repaidLoans.length} completed</strong>
                </div>
                <div className="p-2 bg-white rounded-lg border border-stone-200/80">
                  <span className="text-stone-500 block">Forgiven / Gift Converted:</span>
                  <strong className="text-purple-700 text-xs">{borrowerHistory.convertedGifts.length} agreements</strong>
                </div>
                <div className="p-2 bg-white rounded-lg border border-stone-200/80">
                  <span className="text-stone-500 block">Active Loans:</span>
                  <strong className="text-blue-700 text-xs">{borrowerHistory.activeLoans.length} active</strong>
                </div>
                <div className="p-2 bg-white rounded-lg border border-stone-200/80">
                  <span className="text-stone-500 block">Overdue Delays:</span>
                  <strong className={`text-xs ${borrowerHistory.overdueLoans.length > 0 ? 'text-amber-700 font-bold' : 'text-stone-600'}`}>
          ...
          {supportType === 'loan' && overdueCheck.isOverdue && (
            <div
              ...
              className="p-3.5 bg-amber-50 border border-amber-300 rounded-xl text-xs text-amber-950 space-y-2.5"
            >
              ...
              <label className="flex items-start gap-2 pt-1 text-[11px] font-semibold text-amber-950 cursor-pointer">
                <input
                  type="checkbox"
          ...
          {supportType !== 'contribution' && (
            <div>
              ...
              <input
                ...
                required
                className="w-full px-3 py-2 text-xs border border-stone-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none font-semibold text-stone-900"
              />
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-blue-50/40 border border-blue-100 rounded-xl">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full px-3 py-2 text-xs border border-stone-300 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none bg-white font-medium"
                />
              </div>
          ...
          {supportType === 'contribution' && (
            <div className="space-y-3 p-3 bg-purple-50/40 border border-purple-100 rounded-xl">
              <div>
                <label className="block text-xs font-semibold text-stone-700 mb-1">
                ...
                <input
                  ...
                  required
                  className="w-full px-3 py-2 text-xs border border-stone-300 rounded-lg focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none bg-white font-medium"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-3 py-2 text-xs border border-stone-300 rounded-lg focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none bg-white font-semibold text-stone-900"
                  />
                </div>
          ...
          {/* Purpose / Context */}
          <div>
            <label className="block text-xs font-semibold text-stone-700 mb-1">Purpose / Context</label>
            <input
              ...
              className="w-full px-3 py-2 text-xs border border-stone-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none font-medium"
            />
          </div>

          {/* Optional Notes */}
          <div>
            <label className="block text-xs font-semibold text-stone-700 mb-1">Notes / Terms (Optional)</label>
            <input
          ...
          <div className="pt-3 flex items-center justify-end gap-2.5 border-t border-stone-100">
            ...
            <button
              ...
              type="submit"
              className="px-5 py-2 text-xs font-semibold bg-[#5A2D0C] hover:bg-[#2F1707] text-[#FFF9EE] rounded-xl transition-colors flex items-center gap-1.5 shadow-xs cursor-pointer"
            >

...
export const Hut4DevsFrame: React.FC<Hut4DevsFrameProps> = ({
  ...
  const variantStyles = {
    card: isDark
      ? 'bg-[#3E200C] text-[#FFF9EE] border border-[#C88D3A]/35 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.5)]'
      : 'bg-[#FFF9EE] text-[#5A2D0C] border border-[#C88D3A]/30 shadow-[0_4px_20px_-4px_rgba(90,45,12,0.08)]',
    surface: isDark
      ? 'bg-[#2F1707] text-[#FFF9EE] border border-[#623416] shadow-sm'
      : 'bg-[#F7F1E7] text-[#5A2D0C] border border-[#EAE0D0] shadow-sm',
    highlight: isDark
      ? 'bg-[#3E200C] text-[#FFF9EE] border-2 border-[#C88D3A] shadow-[0_0_25px_-5px_rgba(200,141,58,0.25)]'
      : 'bg-[#FFF9EE] text-[#5A2D0C] border-2 border-[#C88D3A] shadow-[0_0_25px_-5px_rgba(200,141,58,0.2)]',
    subtle: isDark
...
export interface BrandedActionButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  isDark?: boolean;
  variant?: 'bmoni' | 'primary' | 'secondary' | 'accent';
  isLoading?: boolean;
  children: React.ReactNode;
}
...
export const BrandedActionButton: React.FC<BrandedActionButtonProps> = ({
  ...
  const variantStyles = {
    ...
    bmoni: isDark
      ? 'bg-[#2563EB] text-white border-b-4 border-[#1E3A8A] hover:bg-[#3B82F6] hover:border-b-4 hover:border-[#1D4ED8] hover:shadow-[0_8px_25px_rgba(59,130,246,0.5)] active:border-b-0 active:translate-y-[1px]'
      : 'bg-[#1D4ED8] text-white border-b-4 border-[#172554] hover:bg-[#2563EB] hover:border-b-4 hover:border-[#1E3A8A] hover:shadow-[0_8px_25px_rgba(37,99,235,0.45)] active:border-b-0 active:translate-y-[1px]',
    // Primary caramel/gold
    primary: isDark
      ? 'bg-[#C88D3A] text-[#2F1707] border-b-4 border-[#8E560C] hover:bg-[#DDA250] hover:shadow-[0_6px_18px_rgba(200,141,58,0.35)] active:border-b-0 active:translate-y-[2px]'
      : 'bg-[#5A2D0C] text-[#FFF9EE] border-b-4 border-[#351A07] hover:bg-[#432108] hover:shadow-[0_6px_18px_rgba(90,45,12,0.3)] active:border-b-0 active:translate-y-[2px]',
    // Secondary ivory / chocolate outline
    secondary: isDark
      ? 'bg-[#2F1707] text-[#E5D3BA] border-b-4 border-[#1A0C04] hover:bg-[#3E200C] hover:text-[#FFF9EE] active:border-b-0 active:translate-y-[2px]'
      : 'bg-[#FFF9EE] text-[#5A2D0C] border-b-4 border-[#D9C8B0] hover:bg-[#F2E8D8] active:border-b-0 active:translate-y-[2px]',
    // Accent semantic button
    accent: isDark
      ? 'bg-[#B77620] text-white border-b-4 border-[#7A4B0A] hover:bg-[#C88D3A] active:border-b-0 active:translate-y-[2px]'
      : 'bg-[#B77620] text-white border-b-4 border-[#8E560C] hover:bg-[#9E6316] active:border-b-0 active:translate-y-[2px]',
  };
